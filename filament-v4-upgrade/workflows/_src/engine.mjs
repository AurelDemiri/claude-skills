// .claude/workflows/_src/engine.mjs
// Canonical engine for the Filament v4 upgrade workflows. NEVER run directly.
// _assemble.mjs inlines everything between the markers below into each entrypoint.
// Import-safe: no top-level statement references a workflow global (agent/args/etc.).
// <engine:start>
...
// <engine:end>
export {
  buildConfig, RULES, KNOWLEDGE, DIMENSIONS, FATAL_PATTERN_IDS,
  INVENTORY_SCHEMA, FINDINGS_SCHEMA, VERDICT_SCHEMA, GATE_SCHEMA, PLAN_SCHEMA, IMPL_SCHEMA,
  severityRank, dedupe, routeForVerify,
  inventory, baselineGate, auditFanOut, verifyFindings, planFrom, implementPlan, runGate, selfHeal, commitPhase,
}
